...
func TestQueryRange(t *testing.T) {
	...
	t.Run("Query gap between chunks", func(t *testing.T) {
		...
		// We expect to get only the second chunk since it extends to 150
		expectedCount := 1
		if len(segments) != expectedCount {
		...
		if len(segments) > 0 && segments[0].ChunkIndex != 1 {
			t.Errorf("Expected ChunkIndex 1, got %d", segments[0].ChunkIndex)
		}
	})
...
func TestGetSegmentsToRead(t *testing.T) {
	...
	t.Run("Read overlapping chunks", func(t *testing.T) {
		segments := st.GetSegmentsToRead(0, 150)
		...
		if len(segments) > 0 && segments[1].ChunkIndex != 1 {
	...
	t.Run("Read non-overlapping chunks", func(t *testing.T) {
		segments := st.GetSegmentsToRead(150, 225)
		
		// We should have two segments:
		// 1. [150, 200) from chunk 1
		// 2. [200, 225) from chunk 2
		expectedCount := 2
		if len(segments) != expectedCount {
		...
		// First segment should be from chunk 1
		if len(segments) > 0 && segments[0].ChunkIndex != 1 {
			t.Errorf("Expected first segment to be from chunk 1, got %d", segments[0].ChunkIndex)
		}
		
		// Second segment should be from chunk 2
		if len(segments) > 0 && segments[1].ChunkIndex != 2 {
			t.Errorf("Expected second segment to be from chunk 2, got %d", segments[1].ChunkIndex)
		}
	})
}
